<template>
	<view class="chatInterface">
					...
					<view class="message-item" v-else>
						...
						<view class="message-item-content" :class="{'self' : message.senderId ==  currentUser.uuid}">
							<view style="display: flex;align-items: center;justify-content: center;flex-direction: column;">
								...
								<view style="font-size: 20rpx;">{{message.senderData.name}}</view>
							</view>
		...
		<view class="action-box" v-if="!videoPlayer.visible && !messageSelector.visible">
			<view class="action-top" style="margin-bottom: 50rpx;">
...
<script>
	...
	export default {
		...
		onReady() {
			...
			// 	title: this.group.roomTitle + '（' + Object.keys(this.groupMembers).length + '）'
			// });
		},
		onShow() {
		...
		onLoad(options) {
			...
			this.$u.api.chatInfoDetail({
				...
				.then(res => {
					this.group = res[0];
					this.group.roomId=this.roomId.toString()
					console.log(this.group);
					uni.setNavigationBarTitle({
						// title: this.group.roomTitle + '（' + Object.keys(this.groupMembers).length + '）'
						title: this.group.roomTitle
					});
			...
			// this.group = restApi.findGroupById(roomId);
			// this.groupMembers = restApi.findGroupMembers(roomId);
		...
		methods: {
			...
			initialGoEasyListeners() {
				...
				this.goEasy.im.on(this.GoEasy.IM_EVENT.MESSAGE_DELETED, (deletedMessages) => {
					deletedMessages.forEach(message => {
						let groupId = message.groupId;
						if (groupId && groupId === this.group.roomId) {
			...
			initRecorderListeners() {
				...
				recorderManager.onStop((res) => {
					...
					let audioMessage = this.goEasy.im.createAudioMessage({
						to: {
							id: this.group.roomId,
							type: this.GoEasy.IM_SCENE.GROUP,
							data: {
								name: this.group.roomTitle,
			...
			showActionPopup(message) {
				...
				if ((Date.now() - message.timestamp) < MAX_RECALLABLE_TIME && message.senderId == this.currentUser.uuid &&
